...
def scale_seg_mask(img1_shape, mask, img0):
    # gain = min(im.shape[1] / im0s.shape[0], im.shape[2] / im0s.shape[1])  # gain  = old / new
    # pad = (im.shape[2] - im0s.shape[1] * gain) / 2, (im.shape[1] - im0s.shape[0] * gain) / 2
    img0 = img0[0] if isinstance(img0, list) else img0
    img0_shape = img0.shape

    gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = old / new
    pad = round((img1_shape[1] - img0_shape[1] * gain) / 2 - 0.1), round(
        (img1_shape[0] - img0_shape[0] * gain) / 2 - 0.1)  # wh padding
    mask_ = mask[int(pad[1]):int(mask.shape[0] - pad[1]), int(pad[0]):int(mask.shape[1] - pad[0])]
    mask_ = cv2.resize(mask_, (img0_shape[1], img0_shape[0]))  # TODO 为啥shape的值反过来了
    return mask_


def scale_seg_mask_cuda(img1_shape, mask, img0):
    img0 = img0[0] if isinstance(img0, list) else img0
    img0_shape = img0.shape
    ...
    # 将mask和pad转换为PyTorch张量，并移动到GPU上
    pad = torch.tensor(pad).to('cuda')
    # 在GPU上执行相应的操作
    mask_ = mask[
            int(pad[1]):int(mask.shape[0] - pad[1]),
            int(pad[0]):int(mask.shape[1] - pad[0])
            ]
    # 使用PyTorch的resize函数
    mask_ = torch.nn.functional.interpolate(mask_.unsqueeze(0).unsqueeze(0).float(),
                                            size=(img0_shape[0], img0_shape[1]),
                                            mode='nearest')
    # mask_ = mask_.squeeze().byte().cpu().numpy()  # 将结果移回CPU
    mask_ = mask_.squeeze().byte()
    return mask_
...
def colorEncode_cuda(labelmap, colors, mode='RGB'):
    # labelmap = labelmap.astype('int')
    labelmap = labelmap.int()
    # labelmap = torch.from_numpy(labelmap).to('cuda')  # 将labelmap移到GPU上

    # 将colors移到GPU上
    colors = torch.tensor(colors).to('cuda')

    labelmap_rgb = torch.zeros((labelmap.shape[0], labelmap.shape[1], 3), dtype=torch.uint8, device='cuda')

    for label in torch.unique(labelmap):
        if label < 0:
            continue
        mask = (labelmap == label).unsqueeze(2).expand_as(labelmap_rgb)
        color = colors[label][None, None, :]
        # 使用广播机制确保mask和color的形状匹配
        labelmap_rgb += mask * color

    if mode == 'BGR':
        return labelmap_rgb[:, :, [2, 1, 0]]  # Reorder channels to get BGR
    else:
        return labelmap_rgb
...
def draw_yolo_boxes(image, bboxes):
    img_h, img_w, _ = image.shape

    for bbox in bboxes:
        x_center, y_center, box_width, box_height = bbox

        # 将 YOLO 格式转换为左上角坐标和宽高
...
def show_batch_img(batch):

    batch_size = len(batch['im_file'])
    if batch_size == 16:
        fig, axs = plt.subplots(4, int(batch_size/4), figsize=(12, 12))
    else:
        fig, axs = plt.subplots(2, int(batch_size/2), figsize=(12, 6))
    axs = axs.flatten()

    for i in range(batch_size):
        img = batch['img'][i].permute(1, 2, 0).cpu().numpy()
        seg_label = batch['seg_label'][i].cpu().numpy()
        det_label = batch['bboxes'][batch['batch_idx'] == i]


        if np.issubdtype(img.dtype, np.floating):
            image_with_masks = (img.copy()*255).astype('uint8')
        else:
            image_with_masks = img.copy()
        image_with_masks[seg_label == 1] = (255, 255, 0)
        # det label
        image_with_masks = draw_yolo_boxes(image_with_masks, det_label)
        axs[i].imshow(image_with_masks)
        axs[i].axis('off')
