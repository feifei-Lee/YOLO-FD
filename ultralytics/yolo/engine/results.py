...
class Results(SimpleClass):
    ...
    def __init__(self, orig_img, path, names, boxes=None, masks=None, probs=None, keypoints=None,
        ...
        self.area_count = area_count if area_count is not None else {}
        self.tracklets_tracked = None
        self.ax = None
        self.hex = ['#FF3838', '#0018EC', '#00FF00', '#344593', '#FF37C7', '#FF701F', '#2C99A8', '#CFD231',
                    '#48193A', '#92CC17', '#3DDB86', '#1A9334', '#00D4BB', '#FFB21D',
                    '#00C2FF', '#6473FF', '#8438FF', '#520085', '#CB38FF', '#FF95C8']
    ...
    def plot(
        ...
        area_count, show_area = self.area_count, area_count
        tracklets_tracked, show_trajectory = self.tracklets_tracked, show_trajectory
        ...
        if pred_boxes and show_boxes:
            for d, area_cnt in list(zip(reversed(pred_boxes), reversed(area_count))):
                c, conf, id = int(d.cls), float(d.conf) if conf else None, None if d.id is None else int(d.id.item())
                name = ('' if id is None else f'id:{id} ') + names[c]
                if area_count and show_area is True:
                    a0 = area_cnt.get(0, 1)
                    a1 = area_cnt.get(1, 0)
                    area_proportion = a1 / (a0 + a1)  # 添加感染面积占比
                    area_proportion = area_proportion * 1.7 * 100  # 1.7 为鱼体表/检测框大小的比值
                    if area_proportion != 0:
                        label = (f'{name} {conf:.2f} area:{area_proportion:.2f}%'
                                 if conf and area_cnt else name) if labels else None
                    else:
                ...
                trajectory = None
                if show_trajectory and tracklets_tracked is not None:
                    for t in tracklets_tracked:
                        if d.id == t.track_id:
                            trajectory = t.history_tracklet
                            # trajectory = np.array(trajectory)
                            # self.ax.plot(trajectory[:, 0] + trajectory[:, 2] / 2,
                            #              trajectory[:, 1] + trajectory[:, 3] / 2,
                            #              range(1, len(trajectory) + 1), label=f'id_{t.track_id}',
                            #              color=self.hex[t.track_id % 20])

                annotator.box_label(d.xyxy.squeeze(), label, color=colors(c % 20, True), trajectory=trajectory)
            # self.ax.legend(loc='upper right')
            # self.ax.set_title('Multiple 3D trajectory')
