...
class Albumentations:
    ...
    def __init__(self, size=640):
        ...
        try:
            ...
            T = [
                A.RandomResizedCrop(height=size, width=size, scale=(0.8, 1.0), ratio=(0.9, 1.11), p=0.05)]  # transforms
            T2 = [
                A.Blur(p=0.01),
                A.MedianBlur(p=0.01),
                A.ToGray(p=0.05),
    ...
    def __call__(self, im, labels, seg_labels, p=1.0):
        """Transforms input image and labels with probability 'p'."""
        if self.transform and self.transform2 and random.random() < p:
            new = self.transform(image=im, bboxes=labels[:, 1:], class_labels=labels[:, 0],
                                 seg_label=seg_labels)  # transformed
            img, bboxes, class_labels, seg_labels = new['image'], new['bboxes'], new['class_labels'], \
                new['seg_label']
            new = self.transform2(image=img, bboxes=bboxes, class_labels=class_labels)
            im, labels = new['image'], np.array([[c, *b] for c, b in zip(new['class_labels'], new['bboxes'])])

        return im, labels, seg_labels
...
def letterboxandsegLabel(im, seg_label, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    """Resize and pad image while meeting stride-multiple constraints."""
    shape = im.shape[:2]  # current shape [height, width]
    if isinstance(new_shape, int):
        new_shape = (new_shape, new_shape)
    ...
    return im, seg_label, ratio, (dw, dh)
...
def random_perspective(im,
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            seg_targets = cv2.warpPerspective(seg_targets, M, dsize=(width, height), borderValue=(0, 0))
        else:  # affine
            im = cv2.warpAffine(im, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            seg_targets = cv2.warpAffine(seg_targets, M[:2], dsize=(width, height), borderValue=(0, 0))
